...
public class gatinhoArma : MonoBehaviour
{
    ...
    {
        ArmaBase();
        if(kitten.minigun) ArmaMinigun();
        if (kitten.torreta) Torreta();
        if (kitten.laser) ArmaLaser();
    }
    ...
    void ArmaBase()
    {
        if(Time.time > ultimoTiroArma + cadenciaArma)
        {
            ...
            projetilArmaBaseInstanciado.GetComponent<projetilGatinho>().setarComponentes(this.kitten, this.gameManager);
    ...
    void ArmaMinigun()
    {
        if (Time.time > ultimoTiroMinigun + cadenciaMinigun && temperaturaMinigun < temperaturaMaximaMinigun)
        {
            foreach(Transform t in minigunTransform)
            {
                ...
                projetilMinigunInstanciado.GetComponent<projetilGatinho>().setarComponentes(this.kitten, this.gameManager);
    ...
    void ArmaLaser()
    {
        if (Time.time > ultimoTiroLaser + cadenciaLaser)
        {
            foreach (Transform t in laserTransform)
            {
                ...
                projetilLaserInstanciado.GetComponent<projetilGatinho>().setarComponentes(this.kitten, this.gameManager);
    ...
    void Torreta()
    {
        ...
        if (Time.time > ultimoTiroTorreta + cadenciaTorreta)
        {
            if(alvoTorreta != null)
            {
                ...
                projetilTorretaInstanciado.GetComponent<projetilTeleguiado>().SetarProjetil(danoTorreta, velocidadeProjetilTorreta, 10, 1, kitten, alvoTorreta);
